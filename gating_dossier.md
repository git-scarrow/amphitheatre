...
| C-3 | **Slope-stability analysis of the ~20° S/SE/E bowl wall** under terraced-seating loading and Scenario E ramp cuts. The wall is a continuous ~20° (≈35%) amphitheatre-grade rake (multi-scale DEM analysis; `REASSESSMENT_MEMO.md` §2), **not** a retaining-wall condition — Scenario E grades access with 8.33% switchback ramps (max cut/fill 0.95/1.59 ft, no walls). Confirms the graded wall is stable under terrace + ramp loading once soils are known — currently **qualitative**. | Geotechnical + structural PE | Confirms graded-slope stability; sets/replaces the side-slope assumption. | `02_grading`, ADA ramps, `01_layout` | ⛔ |
...
| H-2b | **Code occupant load** (Michigan Building Code / IBC assembly — egress/aisle/exit capacity, fixed vs. movable seats, standing/lawn) and **operational event cap** (per-event, fire-marshal/permit). The Scenario E validated tiers (**1,243 baseline / 1,357 modest / 1,505 ambitious** Band-A seats) are **geometric planning estimates only** — neither occupant load nor event cap has been computed. | Architect + code official (AHJ); operator + fire marshal | `05` seat count → permitted capacity |
...
| H-7 | Spillway **weir C 2.6**, crest 612.0, L 25–30 ft; rim-cut ~1,700 CY (≈30% tolerance) — refine on a surveyed profile. | Civil PE | `07` |
